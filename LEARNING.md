...
```
                    [你在 VSCode 里用 Claude Code]
                              ...
                              ▼
   noapet_hook.py  ── stdin JSON(session_id) ──►  TCP:8787 (localhost)
   (Claude 每次事件启动一个轻量进程)                    │
                              │ 守护没起就自动拉起        ▼
                    noapet_daemon.py  (常驻，独占串口)
...
│ NoaPet         │  大字 (textSize 2): work+wait 合并列表
...
| **hook 必须快速返回** | noapet_hook.py 任何异常都 `sys.exit(0)`，绝不阻塞 Claude |
| **事件 → 状态映射** | UserPromptSubmit→working、Stop→done/all_done、Notification(idle_prompt)→idle、choosing→choosing |
| **ESC 中断的坑** | 按 ESC 不触发任何 hook，导致卡 working。唯一可靠恢复信号是 `idle_prompt`（输入态空闲 60s）→ 用 `matcher=idle_prompt` 显式传 `NotificationIdle` |
| **session_id 关联** | hook 从 stdin 拿 session_id，daemon 用它区分多窗口 |
| **终端命名链路** | PowerShell `claude-pass <名字>` → `$env:NOAPET_NAME` → hook payload → daemon `names` 字典 → `#work` 下发 |
...
### 4.2 终端命名
- 名字优先级: `rename` 命令 > `$NOAPET_NAME` 环境变量 > cwd 目录名
- rename 通过 `noapet_send.py rename "name"` 触发
...
- 心情 +5
- **防连按守卫**：`g_pokeActive` 为 true 时忽略后续 BtnA，防止反复切 GIF
...
- **一键打包**：`pack.bat` 用 Python `zipfile` 模块将必要文件打包为 `NoaPet.zip`（排除构建产物、IDE 配置、备份文件、日志等）。
...
| `src/main.cpp` | 固件：GIF 动画池渲染 + 三页面 UI(MAIN/DETAIL/TOKEN) + 串口协议 + 心情系统 + 戳一戳 |
...
| `tools/noapet_daemon.py` | 常驻守护：独占串口 + TCP 服务 + 多线程 + 自动重连 + 成就/节日/token/统计 |
| `tools/noapet_hook.py` | 轻量桥接：Claude 事件 → TCP 上报，自动拉起 daemon |
| `tools/noapet_send.py` | 手动发状态/改名/调试 CLI |
